...
업무 화면은 필터 UI를 직접 만들지 않고 `TableGrid`에 `filters`만 전달한다.
...
관리자 목록 화면은 `TableGrid`를 사용한다. (`@/components/common/TableGrid`)

`FilterDefinition`, `buildFilterConditions`, `FilterValues`도 같은 파일에서 import한다.
...
추가 버튼이나 다운로드 버튼은 업무 화면에서 `Button`, `Plus`, `Download`를 직접 조립하지 않고 `TableGrid`의 props로 처리한다.
...
`pageSizeOptions`는 기본값 `[20, 50, 100]`이 적용되어 있으며, 이 기본값은 `@/lib/constants`의 `DEFAULT_PAGE_SIZE_OPTIONS`에서 관리한다. 기본값과 다른 건수가 필요할 때만 명시한다.
