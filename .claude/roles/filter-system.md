...
필터 UI는 업무 화면에서 직접 만들지 않는다. `TableGrid`에 `FilterDefinition[]`만 전달하면 드롭다운 + 체크박스/라디오 + 선택 건수 배지가 자동으로 렌더링된다.

`FilterDefinition`, `buildFilterConditions`는 `@/components/common/TableGrid`에서 import한다.
...
type FilterField = "kind" | "mainOpenYn" | "activeYn";

const filterDefinitions = useMemo<FilterDefinition<FilterField>[]>(
  () => [
    { label: "구분",    field: "kind",       options: kindCode.options },
    { label: "메인노출", field: "mainOpenYn", options: openYnCode.options },
    { label: "활성",    field: "activeYn",   options: activeYnCode.options },
  ],
  [kindCode.options, openYnCode.options, activeYnCode.options],
);
```

`mode`를 생략하면 다중 선택(checkbox), `mode: "single"`이면 단일 선택(radio)으로 동작한다.

`buildFilterConditions(list.filters)`는 `FilterValues` → `FilterCondition[]` 변환을 담당하며 API 요청에 그대로 전달한다.
...
4. **프론트엔드** — `FilterDefinition[]`에 항목 추가, `FilterField` 타입에 필드명 추가
