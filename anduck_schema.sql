...
CREATE TABLE "village_profile" (
  ...
  "email"       TEXT,
  "image_ids"   BIGINT[] NOT NULL DEFAULT ARRAY[]::BIGINT[],
  "open_yn"     CHAR(1) NOT NULL DEFAULT 'Y' CONSTRAINT "village_profile_open_yn_chk" CHECK ("open_yn" IN ('Y','N')),
  "created_at"  TIMESTAMP(3) NOT NULL DEFAULT current_kst_timestamp(),
  "created_by"  BIGINT,
  "updated_at"  TIMESTAMP(3) NOT NULL DEFAULT current_kst_timestamp(),
  "updated_by"  BIGINT
...
COMMENT ON TABLE  "village_profile"               IS '마을소개 목록 (다중 행, 공개화면 등록순 표시)';
COMMENT ON COLUMN "village_profile"."id"          IS '고유 ID';
COMMENT ON COLUMN "village_profile"."name"        IS '소개 제목';
COMMENT ON COLUMN "village_profile"."description" IS '소개 본문';
COMMENT ON COLUMN "village_profile"."address"     IS '주소 JSON (오시는 길 용도, nullable)';
COMMENT ON COLUMN "village_profile"."latitude"    IS '위도 (지도 마커)';
COMMENT ON COLUMN "village_profile"."longitude"   IS '경도 (지도 마커)';
COMMENT ON COLUMN "village_profile"."phone"       IS '전화번호';
COMMENT ON COLUMN "village_profile"."email"       IS '이메일';
COMMENT ON COLUMN "village_profile"."image_ids"   IS '이미지 ID 목록 (image.id 소프트 참조)';
COMMENT ON COLUMN "village_profile"."open_yn"     IS '공개 여부 (Y: 공개, N: 비공개)';
